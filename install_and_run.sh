...
TARGET_BOT="$(prompt "Введите @username вашего бота для уведомлений (из BotFather)")"
while [[ ! "$TARGET_BOT" =~ ^@ ]]; do
  TARGET_BOT="$(prompt "Нужно начать с @. Повторите ввод @username бота")"
done
...
config.setdefault("notification", {})["target_bot_username"] = "$TARGET_BOT"
