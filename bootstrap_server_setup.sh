...
DEFAULT_REPO_URL="https://github.com/example/realtor-userbot.git"
...
if [[ "$REPO_URL" == "$DEFAULT_REPO_URL" ]]; then
  echo "⚠️  Укажите ваш URL репозитория через --repo-url (сейчас стоит шаблон)."
  echo "Пример:"
  echo "bash bootstrap_server_setup.sh --repo-url https://github.com/<you>/<repo>.git"
  exit 1
fi
