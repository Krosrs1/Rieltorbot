...
def validate_runtime_config(config: dict[str, Any]) -> None:
    ...
    if target_id is None:
        raise ValueError("config.notification.target_telegram_id is required")
    try:
        int(target_id)
...
async def run_bot(args: argparse.Namespace) -> None:
    ...
    notify_id = int(config["notification"]["target_telegram_id"])
    ...
    async def handler(event):  # noqa: ANN001
        ...
        try:
            ...
            if decision.is_lead:
                ...
                try:
                    await client.send_message(notify_id, build_notification(event, decision), parse_mode="markdown")
